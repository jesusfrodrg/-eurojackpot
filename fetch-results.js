...
async function fetchResults() {
  ...
  // Obtener bote de juegosonce.es
  try {
    var html = await get('https://www.juegosonce.es/resultados-eurojackpot');
    var m = html.match(/([0-9]+[.,][0-9]+|[0-9]+)\s*[Mm]illones/);
    if (!m) m = html.match(/bote[^0-9]{0,30}([0-9]+[.,][0-9]+|[0-9]+)\s*[Mm]ill/i);
    if (!m) m = html.match(/([0-9]+)\s*\.?\s*000\s*\.?\s*000/);
    if (m) {
      bote = m[1].replace(',', '.') + ' millones de euros';
  ...
  // Obtener resultados de euro-jackpot.net
  try {
